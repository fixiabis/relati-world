...
const RelatiGameBasicRule = {
  ...
  validateIsPlayerCanDoPlacement(
    game: RelatiGame,
  ...
  disableAllPieces(game: RelatiGame) {
  ...
  enableAllPieces(game: RelatiGame) {
    ...
    for (let sourceGrid of game.playerSourceGrids) {
      RelatiGameBasicRule.enableNearbyPiecesByGrid(sourceGrid);
    }
  },

  enableNearbyPiecesByGrid(grid: RelatiGrid) {
    ...
    for (let route of RelatiGameBasicRule.RelatiRoutes) {
      const [targetGrid, ...middleGrids] = route.map((direction) =>
        grid.getGridTo(direction)
      ) as HasPieceRelatiGrid[];

      const isTargetGridNotExistsOrHasNotPieceOrPieceNotDisabled = !targetGrid
        ?.piece?.disabled;
      ...
      if (targetGrid.piece.symbol === sourceGrid.piece.symbol) {
        RelatiGameBasicRule.enableNearbyPiecesByGrid(targetGrid);
      }
    }
  },
};
