...
const Play: PageType<Props> = ({
  type = 'x9',
  level,
  roundId,
  withPlayer: opponentOfPlayer,
  rounds,
  mode,
  playerOApi,
  playerXApi,
  ...
  const [isGameOverMessageBoxOpen, setIsGameOverMessageBoxOpen] = useState(
    true
  );
  const [isGameLeaveMessageBoxOpen, setIsGameLeaveMessageBoxOpen] = useState(
    false
  );
  ...
  playerOApi =
    playerOApi || (opponentOfPlayer === 0 ? '/api/next-step' : versusApi);
  playerXApi =
    playerXApi || (opponentOfPlayer === 1 ? '/api/next-step' : versusApi);
  ...
  return (
    <Page
      id="play"
      title={`play${rounds > 1 ? ` (${roundWinsOfO}:${roundWinsOfX})` : ''}`}
    >
...
Play.getInitialProps = async ({
  ...
  return {
    ...
    mode: '1p' in query ? '1p' : '0p' in query ? '0p' : '2p',
