...
const HowToPlay: PageType<Props> = ({ size = 'x5', scene = '1' }) => {
  ...
  const [
    isTutorialLeaveMessageBoxOpen,
    setIsTutorialLeaveMessageBoxOpen,
  ] = useState(false);
  const effectSetting = useSelector<State, SettingState['effect']>(
    (state) => state.setting.effect
  );
  const tutorialSetting = useSelector<State, SettingState['tutorial']>(
    (state) => state.setting.tutorial
  );
  const leavePage = () => router.replace('/legacy/choose-mode?for=tutorial');
  const finishTutorial = () => setIsTutorialFinish(true);
  const openTutorialLeaveMessageBox = () =>
    setIsTutorialLeaveMessageBoxOpen(true);
  const closeTutorialFinishMessageBox = () => setIsTutorialFinishBoxOpen(false);
  const closeTutorialLeaveMessageBox = () =>
    setIsTutorialLeaveMessageBoxOpen(false);
...
export default convertPage(HowToPlay);
