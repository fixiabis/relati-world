...
const RelatiGameBy0Player: PlayGameComponent = ({
  type: size,
  opponentOfPlayer,
  playerOApi,
  playerXApi,
  ...
  useEffect(() => {
    ...
    const apiUrlWithQuery = `${apiUrl}${apiUrl.includes('?') ? '&' : '?'}turn=${
      game.turn
    }&pieces=${pieceCodes}&level=${level}`;
    ...
    Axios.get(apiUrlWithQuery)
      ...
      .then(() => {
        ...
        if (game.isOver) {
          ...
          const pieceCodes = convertBoardToPieceCodes(game.board);
          const playerOApiWithQuery = `${playerOApi}${
            playerOApi.includes('?') ? '&' : '?'
          }turn=${game.turn}&pieces=${pieceCodes}&level=${level}`;
          const playerXApiWithQuery = `${playerXApi}${
            playerXApi.includes('?') ? '&' : '?'
          }turn=${game.turn}&pieces=${pieceCodes}&level=${level}`;
  ...
  return <RelatiGame {...props} game={game} onGridClick={handleGridClick} />;
};
